...
performance (``--worker`` already ran the solve GIL-free in-VIO).
...
* :mod:`ba.engine` -- the in-process / subprocess runners that drive the heavy
  keyframe solve (this is now the SINGLE home -- ``vio.engine`` was deleted once the
  backend moved here; the ``ba`` process drives only the in-process engine). The
  algorithm itself lives in the shared :mod:`sky.backend` / :mod:`sky.vio` libraries.
