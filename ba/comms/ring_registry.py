...
A :class:`comms.shared_array.SharedArrayRing` must be created with a FIXED shape +
dtype so the producer and every consumer can attach to the same memory layout.
The capture process publishes ~3 numpy streams (gray_left, gray_right, depth_m);
VIO additionally publishes (gray_left, depth_m, track_ids/track_px implicit in
keyframe). All consumers of one process need to know the same registry to attach
correctly.

This module centralises that registry so a wiring mistake (subscriber attaches to
wrong shape) is caught at process-boot, not at first frame.
...
from .shared_array import SharedArrayRing
...
# Other endpoints register their own as needed (see the per-project mains).
# --------------------------------------------------------------------------- #
def default_capture_specs(endpoint: str = "oak.capture",
    ...
    * ``<endpoint>.depth_m`` -- metric depth (rectified-left grid), float32 (H, W).
    ...
    ``slots=64`` must stay strictly greater than the IPCPubSub outbound cap
    (default 32) so that a wire message queued in a subscriber's outbox still
    points at a slot the producer has not yet overwritten.
    """
...
def default_vio_specs(endpoint: str = "oak.vio",
    ...
    ``frame.tracks`` carries ONLY the per-frame ids + pixels (pure POD); the gray
    + depth needed to render the overlay are read from capture's ``FRAME_DEPTH``
    (capture is the single writer of those rings). So no image / depth ring lives
    here for the tracks topic.
    """
    h, w = int(height), int(width)
    return [
