...
> `MapWindow` (SLAM Map 3D room), `RoomBlocksWindow` (Room Blocks 3D voxel), and the two
...
| **RoomBlocksWindow (Room Blocks 3D voxel)** `ui/qt/room_blocks_window.py` + `ui/viz/voxel_blocks.py` (`occupancy_voxels` / `cube_mesh`, UI-only), fed by `IpcVoxelMapSource` (`ui/modules/ipc_sources.py`) | Grid + ENU triad; the SAME mapped space as the SLAM Map but as a **solid-voxel occupancy grid**. Every VIO keyframe's DENSE depth is back-projected by its OWN pose `[R \| t]` (valid-depth band `[0.3, 6.0] m` + a 4-neighbour `EDGE_MAX_M` flying-pixel reject, sub-sampled by `STRIDE`=3), binned into a `VOXEL_M` (≈0.12 m) grid; a cell is OCCUPIED only with ≥ `MIN_HITS` (=3) points (multi-keyframe agreement → rejects thin stereo noise). The occupied cell centres are merged into ONE shaded `GLMeshItem` (`M` voxels → `M·8` verts / `M·12` triangles, `shader='shaded'`, `smooth=False`), face-coloured by height (optical-world `+y` = DOWN) so floor/walls/ceiling read distinctly. Rebuilt off-thread at ≈2.5 Hz (a cube-mesh rebuild is heavier than a scatter); ≈20 k voxels / ≈250 k triangles / ≈250 ms on the gold sessions at `VOXEL_M`=0.12. SAME ENU frame + camera as `Viewer3D` / `MapWindow`, so the two 3D windows are comparable. | **Blocky occupancy reconstruction** of the keyframe stage — walls/surfaces as solid cube shells with the empty interior visible, COMPLEMENTING the sparse landmark map. Consumes vio `keyframe` (gray/depth via VIO's kf rings) only — pure consumer, no data-path change. Shares the `_KeyframeAccumulator` base (SHM/recv wiring) with `IpcSlamMapSource`. |
