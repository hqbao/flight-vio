...
> `MapWindow` (SLAM Map 3D room), `RoomSurfaceWindow` (Room Surface 3D mesh), and the two
...
| **RoomSurfaceWindow (Room Surface 3D mesh)** `ui/qt/room_surface_window.py` + `ui/viz/surface_mesh.py` (`select_spread_keyframes` / `depth_surface_mesh`, UI-only), fed by `IpcSurfaceMapSource` (`ui/modules/ipc_sources.py`) | Grid + ENU triad; the SAME mapped space as the SLAM Map but as **continuous shaded surfaces** (a per-keyframe depth-map mesh — NO marching-cubes / Poisson lib; deps stay numpy + cv2 + pyqtgraph for the C port). A spatially-spread keyframe subset is picked (greedy: keep a keyframe only if its camera is > `KF_SPACING_M` (≈0.3 m) from every kept one, so a few viewpoints cover the room). Each selected keyframe's depth map (sub-sampled by `MESH_STRIDE`=3, valid-depth band `[0.3, 6.0] m`) is back-projected by its OWN pose `[R \| t]` to a world-frame vertex grid and **triangulated**: every 2×2 cell → 2 triangles, KEPT only when all 4 corners are valid AND their (camera-frame) depth spread ≤ `EDGE_MAX_M` (≈0.1 m) — which drops the fg/bg "curtain" triangles across depth discontinuities (essential, or the room fills with sheets). The per-keyframe meshes are stacked (face indices offset by the running vertex count) into ONE `GLMeshItem` (`shader='shaded'`, `smooth=True`), vertices coloured by height (optical-world `+y` = DOWN, modulated by gray) so floor/walls/ceiling read as a gradient. Total triangles capped at `MAX_TRIANGLES` (≈1.5 M); past it the build coarsens `MESH_STRIDE` and logs it. Rebuilt off-thread at ≈2.5 Hz; on the gold sessions ≈2–10 selected keyframes → ≈7 k–57 k triangles → ≈2–10 ms at `MESH_STRIDE`=3. SAME ENU frame + camera as `Viewer3D` / `MapWindow`, so the two 3D windows are comparable. | **Continuous-surface reconstruction** of the keyframe stage — walls/floor/ceiling as smooth connected surfaces, COMPLEMENTING the sparse landmark map. Consumes vio `keyframe` (gray/depth via VIO's kf rings) only — pure consumer, no data-path change. Shares the `_KeyframeAccumulator` base (SHM/recv wiring) with `IpcSlamMapSource`. |
