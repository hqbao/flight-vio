...
Status: **IN PROGRESS — S0–S4 DONE** (`sky/` scaffolded, `skymath` re-homed as
`sky.math`, SGM stereo → `sky.depth.stereo`, PnP → `sky.front.pnp`, IMU gyro/accel
calib → `sky.sensors`, loose IMU preint → `sky.imu.imu`; oracle `gap=0` preserved at
every step). S5–S6 still pending; S7 deferred until Phase 4 freezes. Goal: pull the DUPLICATED
