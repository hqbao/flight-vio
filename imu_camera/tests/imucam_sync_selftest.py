...
Wires the real modules -- :class:`~imu_camera.modules.read_cam.ReadCamModule` and
:class:`~imu_camera.modules.pipeline.ImuCamModule` -- over a real
:class:`~imu_camera.comms.LocalPubSub`, fed by a recorded gold session (no
device), and verifies the synchronisation contract end to end:

* one :class:`~imu_camera.comms.messages.ImuCamPacket` per camera frame,
...
Run::
    ...
    python -m imu_camera.tests.imucam_sync_selftest
    python -m imu_camera.tests.imucam_sync_selftest --session sessions/gold/lab_loop_30s
"""
...
from imu_camera.comms import LocalPubSub, Module, topics             # noqa: E402
from imu_camera.io.reader import SessionReader                       # noqa: E402
from imu_camera.modules.pipeline import ImuCamModule                 # noqa: E402
from imu_camera.modules.read_cam import (                            # noqa: E402
    ReadCamModule, ReplayCamSource)
from imu_camera.modules.read_imu import ReplayImuSource              # noqa: E402


class _Collector(Module):
...
def run(session: str, max_frames: int) -> list:
    ...
    imu_module = ImuCamModule(bus, ReplayImuSource(reader))
