...
def test_no_ba() -> None:
    ...
    argv = build_vio_args(_ns(no_ba=True), cap, vio, slam,
...
def test_build_ba_args() -> None:
    ...
    # (d) loose ba -> --vio-endpoint / --endpoint only, NO --tight.
    ...
    # (e) tight ba -> --tight appended.
    argv = build_ba_args(_ns(tight=True), vio, ba)
    assert "--tight" in argv, argv
    print("[e] build_ba_args (tight)  -> '--tight' added                          OK")
...
def test_no_slam() -> None:
    ...
    argv = build_vio_args(_ns(tight=True, no_slam=True), cap, vio, slam)
