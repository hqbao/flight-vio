...
:class:`~ours.flows.imu_cam.admission.Admission` that a frame finished, so the
:class:`~ours.flows.imu_cam.admit_frame.AdmitFrame` gate can admit the next.

It runs on the imu-cam flow's own thread (same inbox as the camera triggers), so
admit/complete are naturally serialised. With the
:class:`~ours.flows.imu_cam.admission.AdmitAll` strategy this is a no-op.
"""
