...
Subscribes ``imucam.sample`` and ``frame.depth`` -- BOTH published by the
``imu_cam`` flow (which packs the synced frame+IMU and computes depth inline).
Owns the IMU->prior fusion itself (``PreintegratePrior``). Publishes ``pose.odom``
every frame and a ``keyframe`` every few frames for the back-end / SLAM flows.
