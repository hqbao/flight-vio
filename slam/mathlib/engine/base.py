...
* :class:`InProcessEngine` -- runs the solve synchronously on the calling thread.
  ...
  flow.
* :class:`~slam.mathlib.engine.subprocess.SubprocessEngine` -- ships each keyframe to
  ...
  ``slam/mathlib/engine/subprocess.py``).
...
class SlamResult:
    ...
    Returned by :func:`slam.mathlib.engine.steps.slam_step` only on a keyframe that
