"""ui tracks module: forward streamed KLT tracks to a callback (keypoints view).

The sink that drives the keypoint-depth tracker window
(:class:`~ui.qt.keypoints_window.KeypointTrackWindow`). It subscribes
``frame.tracks`` -- the REAL frontend tracks the odometry module publishes -- and
``frame.depth`` (the matching rectified-left image + metric depth published by
the ``imu_cam`` module), joins them by ``seq``, and hands a
...
producer side (odometry's ``PublishTracks``) ships ONLY the per-frame tracks
dataclass -- the gray + depth are written once by capture into its shared-memory
rings (single writer), instead of VIO racing capture to overwrite the same slots
...
-- and forwards each :class:`~ui.comms.messages.FrameInliers` to it (step in
:mod:`ui.modules.render_inliers`), so the window can mark the clean subset the
...
class TracksWithFrame:
    ...
    Built inside :class:`UiTracksModule` by joining
    :class:`~ui.comms.messages.FrameTracks` (ids + pixels, from odometry)
    with :class:`~ui.comms.messages.DepthFrame` (gray + depth, from imu_cam)
...
class UiTracksModule(Module):
    """Sink module: join ``frame.tracks`` + ``frame.depth`` by seq, forward to ``on_tracks``."""

    def __init__(self, bus: LocalPubSub,
                 on_tracks: Callable[[TracksWithFrame], None], *,
