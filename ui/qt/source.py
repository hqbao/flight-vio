...
The viewer ([ui.qt.mainwindow.MainWindow]) consumes 6-DoF poses from anything
implementing :class:`PoseSource`: it ``start(callback)``s the source, which runs
a background thread and pushes :class:`~ui.comms.lib.misc.pose.Pose` samples (NED)
to the callback. This is the UI's input contract -- it deliberately knows nothing
about how the poses are produced.
...
The real proc4 live source is :class:`ui.main.IpcPoseSource`, which subscribes to
VIO's ``pose.odom`` over IPC and bridges its poses to this contract.
"""
...
from ui.comms.lib.misc.frames import rpy_to_quat
