...
TRACE_PATH  = "#7cff5c"   # VIO (pose.odom) trajectory line — NVG green. The
                          # responsive frame-to-frame live trail + the marker.
VO_PATH     = "#8b949e"   # VO (pose.vo, pure vision) line — DIM desaturated grey.
                          # Reads as the "raw / noisy" stream that drifts most;
                          # deliberately muted so it sits behind the brighter VIO.
BA_PATH     = "#7aa2ff"   # VIO-BA (pose.refined, windowed-BA keyframes) line —
                          # violet-blue. Distinct from green VIO, grey VO, cyan
                          # SLAM, orange corrected, and the magenta/red teleport.
REFINED_PATH = "#5ce1ff"  # SLAM keyframe map line — HUD cyan, reads as the
                          # corrected map BEHIND the live green marker path
CORRECTED_PATH = "#ff8c42"  # "SLAM-corrected VIO" line — warm orange. The dense
                          # VIO trail rubber-sheeted by SLAM's loop corrections.
                          # Distinct from green VIO, grey VO, blue BA, cyan SLAM,
                          # the amber kf dots, and NOT the teleport colour.
TELEPORT    = "#ff3b30"   # teleport flash — master-warning red. Marks the
                          # corrected-VIO vertices SLAM pulled/snapped far on a
                          # loop closure (|correction_delta| > TELEPORT_M), so a
                          # large map correction reads as a flagged "swoosh".
