...
Runs right after :class:`~vio.modules.estimate_motion.EstimateMotion` in the
...
would misrepresent "no disagreement" as a real measurement. The :class:`Step` is
always passed through unchanged so ``PublishPose`` / ``EmitKeyframe`` still run.
"""
...
class PublishGyroFuse(StepBase):
    name = "publish_gyrofuse"
